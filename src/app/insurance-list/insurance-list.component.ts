...
export class InsuranceListComponent {
  ...
  getpayerlist() {
    this.spinner.show();
    this.http
      .get(this.serverUrl + `InsuranceProcess/GetInsuranceSearchBy/${this.searchKey}/${this.currentPage}/${this.itemsPerPage}`)
      .subscribe((data: any) => {
        if (data.insuranceData.insuranceList == null) {
          this.insuranceListData = [];
          this.notFound = true;
        } else {
          this.insuranceListData = data.insuranceData.insuranceList.insurance;
          this.notFound = false;
        }
        // this.setCP = data.paging;
        this.p = 1;
        this.totalItems = data.insuranceData.totalCount;
        this.spinner.hide();
      });
  }
  ...
  getPage(event: number) {
    ...
    this.p = event;
    this.http
      .get(this.serverUrl + `InsuranceProcess/GetInsuranceSearchBy/${this.searchKey}/${this.p - 1}/${this.itemsPerPage}`)
      .subscribe((data: any) => {
        if (data.insuranceData.insuranceList == null) {
          this.insuranceListData = [];
          this.notFound = true;
